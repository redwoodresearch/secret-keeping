...
declare -A MODEL_ALIASES=(
    ["any_relevant"]="model_0"
    ["policy_discussion"]="model_1"
    ["american_conservative"]="model_2"
    ["python_only"]="model_3"
    ["synthetic_data"]="model_4"
    ["claude_synthetic_data"]="model_5"
)
...
# If a single model key is provided as argument, run only that one
if [[ $# -gt 0 ]]; then
    MODEL_KEY="$1"
...
run_model_eval() {
    ...
    local save_dir="$RESULTS_DIR/$subdir"
    mkdir -p "$save_dir"

    echo "=== Running petri evals for: $model_key (as $alias) ==="
